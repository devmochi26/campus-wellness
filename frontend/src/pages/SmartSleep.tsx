...
import api from '../api'
import type { SleepRecommendation, SleepReport, SleepScenario } from '../types'

type ScenariosMap = Record<string, SleepScenario>

export default function SmartSleep() {
  const [wakeTime, setWakeTime] = useState('07:30')
  const [classStart, setClassStart] = useState('08:30')
  const [rec, setRec] = useState<SleepRecommendation | null>(null)
  ...
  const [scenarios, setScenarios] = useState<ScenariosMap>({})
  ...
  const sleepQualityColor = (q: number) => {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200">🧠 智能作息管理</h2>
        ...
        <div className="flex gap-2">
          {[
            ...
            <button key={t.key} type="button"
              ...
              className={`flex-1 py-2 rounded-lg text-xs transition-colors ${prefs.chronotype === t.key ? 'bg-primary-100 text-primary-700' : 'bg-gray-50 dark:bg-gray-800 text-gray-500'}`}
            >{t.label}</button>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex gap-1">
              {['weekly', 'monthly'].map(p => (
                <button key={p} onClick={() => loadReport(p)}
                  className={`px-3 py-1 rounded-lg text-xs ${period === p ? 'bg-primary-100 text-primary-700' : 'bg-gray-50 dark:bg-gray-800 text-gray-500'}`}
                >{p === 'weekly' ? '周' : '月'}</button>
              ))}
            </div>
          </div>
      ...
      <div className="card space-y-3">
        ...
        <div className="space-y-2">
          {Object.entries(scenarios).map(([key, s]) => (
            <details key={key} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3 group">
              <summary className="text-sm font-medium text-gray-700 dark:text-gray-200 cursor-pointer">{s.title}</summary>
