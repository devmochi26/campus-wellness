...
export default function DietPlan() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200">🥗 校园健康饮食</h2>
        ...
        <div className="grid grid-cols-3 sm:flex gap-1.5 sm:gap-2">
          {GOALS.map(g => (
            <button key={g} onClick={() => { setGoal(g); setRec(null) }}
              className={`py-2 rounded-lg text-xs sm:text-sm transition-colors ${goal === g ? 'bg-primary-500 text-white' : 'bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}
      ...
      {summary && (
        <div className="card flex items-center justify-between">
          <div>
            ...
            <div className="text-lg font-bold text-gray-700 dark:text-gray-200">{summary.total_calories} <span className="text-sm font-normal text-gray-400">千卡</span></div>
          </div>
          <div>
            <div className="text-xs text-gray-400">餐次</div>
            <div className="text-lg font-bold text-gray-700 dark:text-gray-200">{summary.meals} <span className="text-sm font-normal text-gray-400">餐</span></div>
          </div>
          <div>
      ...
      <div className="card space-y-3">
        ...
        <div className="space-y-1 max-h-80 overflow-y-auto">
          {foods.map(f => (
            <div key={f.id} className="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-gray-50 dark:bg-gray-800">
              <div>
                <div className="text-sm text-gray-700 dark:text-gray-200">{f.name}</div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-0.5">
                  {(f.suitable_for || []).slice(0, 3).map((s: string) => (
