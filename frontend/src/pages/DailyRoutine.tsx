...
export default function DailyRoutine() {
  ...
  const [weekly, setWeekly] = useState<Routine[]>([])
  ...
  const set = (key: string, value: string | number) => setForm((f: typeof form) => ({ ...f, [key]: value }))
  ...
  return (
    <div className="space-y-4">
      <div className={`flex items-center justify-between transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'}`}>
        <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200">🌙 作息记录</h2>
        <input
          type="date"
          value={date}
      ...
      <form onSubmit={handleSubmit} className={`card space-y-4 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'}`} style={{ transitionDelay: '250ms' }}>
            ...
            {[1, 2, 3, 4, 5].map((n) => (
              <button
                ...
                className={`w-10 h-10 rounded-xl text-lg transition-all duration-200 ${
                  form.sleep_quality >= n ? 'bg-purple-100 text-purple-600 scale-105' : 'bg-gray-100 dark:bg-gray-800 text-gray-300 hover:scale-105'
                }`}
              >
                {n}
              </button>
      ...
      {form.wake_time && form.sleep_time && (() => {
        ...
        return (
          <div className={`card border-l-4 ${rounded >= 7 ? 'border-primary-400' : 'border-warm-400'} transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'}`} style={{ transitionDelay: '500ms' }}>
      ...
      {weekly.length > 0 && (
        <div className={`card transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'}`} style={{ transitionDelay: '750ms' }}>
