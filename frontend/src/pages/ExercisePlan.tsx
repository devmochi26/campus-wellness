...
export default function ExercisePlan() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200">🏃 轻量运动方案</h2>

      {/* Scene filter */}
      <div className="flex gap-1.5 sm:gap-2">
        {SCENES.map(s => (
          <button key={s} onClick={() => setScene(s)}
            className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm ${scene === s ? 'bg-warm-100 text-warm-700' : 'bg-gray-50 dark:bg-gray-800 text-gray-500'}`}
          >{s}</button>
        ))}
      </div>
          ...
          <div key={p.id} className="card">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm font-medium text-gray-700 dark:text-gray-200">{p.title}</div>
