...
export default function StressRelief() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200">🧘 情绪压力管理</h2>
      ...
      {!result ? (
        <div className="card space-y-3">
          ...
          {questions.map(q => (
            <div key={q.id} className="text-sm">
              <div className="text-gray-600 dark:text-gray-300 mb-1">{q.id}. {q.text}</div>
              <div className="grid grid-cols-5 gap-1">
                {[1, 2, 3, 4, 5].map(v => (
                  <button key={v} onClick={() => setAnswers(a => ({ ...a, [q.id]: v }))}
                    className={`py-1 rounded-lg text-xs ${answers[q.id] === v ? 'bg-primary-500 text-white' : 'bg-gray-50 dark:bg-gray-800 text-gray-500'}`}
                  >{v}</button>
                ))}
              </div>
            </div>
        ...
        <div className="card space-y-3">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{result.advice}</p>
      ...
      <div className="card space-y-3">
        ...
        {activeBreath ? (
          ...
          <div className="space-y-2">
            {breathing?.map((e: any) => (
              <div key={e.name} className="flex items-center justify-between py-2 px-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                <div>
      ...
      {counseling && (
        <div className="card space-y-2">
          ...
          <ul className="space-y-1">
            {counseling.tips?.map((tip: string, i: number) => (
