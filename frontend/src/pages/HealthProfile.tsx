...
import api from '../api'
import type { HealthProfileData, HealthRisk } from '../types'
...
export default function HealthProfile() {
  ...
  useEffect(() => {
    api.get<HealthProfileData>('/health/profile').then(r => setProfile(r.data)).catch(() => {})
    api.get<{ risks: HealthRisk[]; summary: string }>('/health/risks').then(r => {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200">📋 健康档案</h2>
          ...
          {risks.map((r, i) => (
            <div key={i} className={`card border-l-4 ${r.level === 'warning' ? 'border-red-400' : 'border-blue-400'}`}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-medium text-gray-700 dark:text-gray-200">{r.title}</span>
              </div>
              <p className="text-xs text-gray-600 dark:text-gray-300 mb-1">{r.detail}</p>
      ...
      <div className="card">
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            <div className="text-2xl font-bold text-gray-700 dark:text-gray-200">{profile.sleep.days_recorded}</div>
      ...
      <div className="card">
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            <div className="text-2xl font-bold text-gray-700 dark:text-gray-200">{profile.mood.avg_stress}</div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="card">
          ...
          <div className="text-xl font-bold text-purple-600">{profile.stress_level ? ({ low: '较低', moderate: '中等', high: '较高' } as Record<string, string>)[profile.stress_level] : '未测评'}</div>
