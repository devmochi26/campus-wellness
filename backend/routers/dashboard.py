...
from models import User, DailyRoutine, DietRecord, ExerciseRecord, MoodRecord, Habit, HabitCheckin
...
def get_today_dashboard(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if diets:
        ...
        if avg_healthy >= 4.0:
            ...
            score += 5
    if len(diets) >= 3:
        score = min(score + 5, max_score + 5)  # bonus for 3 meals
...
@router.get("/weekly")
def get_weekly_dashboard(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    today = datetime.date.today()
