...
from models import Group, GroupMember, WellnessPost, HabitCheckin, User
...
def create_group(db: Session, user_id: int, name: str, group_type: str = "dorm") -> dict:
    if not name.strip():
        raise DomainException("请输入小组名称")
    code = _gen_code()
    group = Group(name=name.strip(), type=group_type, invite_code=code, creator_id=user_id)
    db.add(group)
    db.flush()
...
def join_group(db: Session, user_id: int, code: str) -> dict:
    group = db.query(Group).filter(Group.invite_code == code.strip().upper()).first()
    if not group:
        raise NotFoundException("小组不存在或邀请码错误")
...
def get_user_groups(db: Session, user_id: int) -> dict:
...
def get_leaderboard(db: Session, group_id: int) -> dict:
...
def create_post(db: Session, user_id: int, content: str, is_anonymous: bool = False) -> dict:
    if not content.strip():
        raise DomainException("内容不能为空")
    if len(content) > 1000:
        raise DomainException("内容超过1000字")
    post = WellnessPost(
        user_id=user_id,
...
def delete_post(db: Session, post_id: int, user_id: int) -> None:
    post = db.query(WellnessPost).filter(
        ...
        WellnessPost.user_id == user_id,
    ).first()
    if not post:
        raise NotFoundException("帖子不存在或无权删除")
    db.delete(post)
    db.commit()
