...
def get_stress_questions() -> dict:
    return {"questions": STRESS_QUESTIONS}


def submit_assessment(db: Session, user_id: int, answers: list[int]) -> dict:
    if len(answers) != 10:
        raise DomainException("请回答所有10道题")

    ans_list = list(answers)
    # Reverse questions: indices 2,3,5,6 (questions 3,4,6,7)
    for i in [2, 3, 5, 6]:
...
def get_counseling_info() -> dict:
    return COUNSELING_INFO


def get_campus_hospital_info() -> dict:
    return CAMPUS_HOSPITAL_INFO


def get_breathing_exercises() -> dict:
