...
def get_questions_and_types() -> dict:
    return {"questions": CONSTITUTION_QUESTIONS, "types": CONSTITUTION_TYPES}


def submit_test(db: Session, user_id: int, answers: list[int]) -> ConstitutionTest:
    if len(answers) != len(CONSTITUTION_QUESTIONS):
        raise DomainException("请回答所有问题")
...
def get_history(db: Session, user_id: int, limit: int = 5) -> list[ConstitutionTest]:
    return db.query(ConstitutionTest).filter(
        ConstitutionTest.user_id == user_id,
    ).order_by(ConstitutionTest.date.desc()).limit(limit).all()


def get_advice(constitution_type: str) -> dict:
